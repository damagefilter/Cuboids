...
public class CmodTpTo extends CBaseCommand {
    ...
    public void execute(CPlayer player, String[] command) {
        ...
        CuboidE targetCube = RegionManager.getInstance().getCuboidByName(command[1], player.getWorld().getName(), player.getWorld().getDimension());
        if(targetCube == null) {
            MessageSystem.getInstance().failMessage(player, "cuboidNotFoundOnCommand");
            return;
        }
        Vector target = Vector.getCenterPoint(targetCube.getFirstPoint(), targetCube.getSecondPoint());
        ...
        if(player.hasPermission("cIgnoreRestrictions") || (player.hasPermission("cteleport") && targetCube.playerIsAllowed(player.getName(), player.getGroups()))) {
            ...
            MessageSystem.getInstance().successMessage(player, "playerTeleported");
        }
        else {
