...
public class CmodRename extends CBaseCommand {
    ...
    public void execute(CPlayer player, String[] command) {
        ...
        CuboidNode node = RegionManager.getInstance().getCuboidNodeByName(command[1], player.getWorld().getName(), player.getWorld().getDimension());
        
        if(!player.hasPermission("cIgnoreRestrictions")) {
            if(!(node.getCuboid().playerIsOwner(player.getName()) || player.hasPermission("cAreaMod"))) {
