...
public class CuboidE {
    ...
    /**
     * Set the parent of this cuboid. Parenting cuboids will affect storage
     * mechanics.<br>
    ...
    /**
     * Set this cuboids priority level. 1 - x where 10 has higher priority than
     * 1
     * 
     * @param priority
     */
    ...
    /**
     * Add a group to the list of allowed groups. This will transform group
     * names to lower case
     * 
     * @param group
     *            String
            ...
            if (group.indexOf("g:") >= 0) {
                if (!group.equalsIgnoreCase("no_groups")) {
                    if (!group.equalsIgnoreCase("g:")) {
                        groupList.add(group.substring(2).replace(" ", "")
                                .toLowerCase());
                        return true;
                    }
                }
    ...
    public boolean commandIsRestricted(String command) {
        if (tabuCommands.contains(command)
                || tabuCommands.contains(command.substring(1))) {
            return true;
        } else {
            return false;
        }
     ...
     *            inside only partially
     * @param world
     *            The World Name to check against
     * @return true if cuboid is inside another, false otherwise
     */
    public boolean cuboidIsWithin(Vector v1, Vector v2, String world,
            int dimension, boolean complete) {
    ...
    public boolean cuboidIsWithin(CuboidE cube, boolean complete) {
        if (this.equalWorlds(cube)) {
            Vector min = Vector.getMinimum(cube.getFirstPoint(),
                    cube.getSecondPoint());
            Vector max = Vector.getMaximum(cube.getFirstPoint(),
                    cube.getSecondPoint());
    ...
    public boolean equalWorlds(CuboidE test) {
        if ((this.getDimension() == test.getDimension())
                && (this.getWorld().equalsIgnoreCase(test.getWorld()))) {
            return true;
        }
        return false;
    }
    ...
    public boolean equalWorlds(String world, int dimension) {
        if ((this.getDimension() == dimension)
                && (this.getWorld().equalsIgnoreCase(world))) {
            return true;
        }
        return false;
    }
    ...
    /**
     * Check if a given player is allowed in this cuboid. That is to say, if his
     * name is inside the list of players.
     * 
     * @param player
    ...
    /**
     * Check if a given group is allowed in this cuboid. That is to say, if his
     * name is inside the list of groups.
     * 
     * @param group
     * @return
     */
    ...
    /**
     * Retrieve the area settings as boolean HashMap Used for copying properties
     * to child cuboids when adding new ones.
     * 
     * @return
     */
    ...
    public int getSize() {
        return (int) Vector.getDistance(point1.getBlockX(), point2.getBlockX())
                * (int) Vector.getDistance(point1.getBlockY(),
                        point2.getBlockY())
                * (int) Vector.getDistance(point1.getBlockZ(),
                        point2.getBlockZ());
    }
    ...
    public void overwriteProperties(HashMap<String, Boolean> props) {
        ...
        setSanctuarySpawnAnimals(((Boolean) props.get("sanctuarySpawnAnimals"))
                .booleanValue());
        setFreeBuild(((Boolean) props.get("freeBuild")).booleanValue());
        setBlockFireSpread(((Boolean) props.get("blockFireSpread"))
                .booleanValue());
